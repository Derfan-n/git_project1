...
"""
    ...
    - заканчивать диалог удаление и добавления в корзину при нажатии кнопки назад
"""
...
markup_menu = ReplyKeyboardMarkup(reply_keyboard_menu, one_time_keyboard=False)
markup_buy = ReplyKeyboardMarkup(reply_keyboard_buy, one_time_keyboard=False)
markup_add = ReplyKeyboardMarkup(reply_keyboard_add_product, one_time_keyboard=False)
markup_favorites_and_basket = ReplyKeyboardMarkup(
    reply_keyboard_favorites_and_basket, one_time_keyboard=False)
markup_delete = ReplyKeyboardMarkup(reply_keyboard_delete, one_time_keyboard=False)
markup_back = ReplyKeyboardMarkup(reply_keyboard_back, one_time_keyboard=False)
...
def basket(update, context):
    ...
    update.message.reply_text('Корзина:', reply_markup=markup_back)
    db_sess = db_session.create_session()
...
def selected_products(update, context):
    global ad_pr, del_product, del_from
    update.message.reply_text('Товары которые находятся в избранном:', reply_markup=markup_back)
    db_sess = db_session.create_session()

    user = db_sess.query(User).filter(User.telegram_id == update.message.from_user.id).first()
    print(user.elected_products)
    if user.elected_products is not None and len(str(user.elected_products)) != 0:
        ...
        update.message.reply_text('Избранных товаров нет')
...
def delete(update, context):
    ...
    ad_pr = []
    product_to_del = ''
    update.message.reply_text('товар, был удалён', reply_markup=markup_buy)
...
def product(update, context):
    global add_product, name_product, category_name
    """Показывает товары в категории technic"""
    category_name = dict_category_of_products[update.message.text]
    print(category_name)
    update.message.reply_text('Список товаров:', reply_markup=markup_buy)
    product_names = []
    name_product = []
    db_sess = db_session.create_session()
...
def product_add(update, context):
    global add_product, added_product, category_name
...
def back(update, context):
    if back_count[0] == 1:
        ...
        back_count[0] = 0
        print(back_count)
    if back_count[1] == 1 and back_count[3] == 0 and back_count[4] == 0:
        update.message.reply_text('Основное окно:', reply_markup=markup)
        back_count[1] = 0
        print(back_count)
    if back_count[4] == 1:
...
def transition(update, context):
    ...
    elif add_product == 1 and (update.message.text in name_product):
        product_add(update, context)

    elif del_product == 1 and (update.message.text in ad_pr):
